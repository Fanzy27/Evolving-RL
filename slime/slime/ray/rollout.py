...
class RolloutManager:
    ...
    def get_updatable_engines_and_lock(self, model_name: str | None = None):
        """Backward-compatible alias for older train actors."""

        return self.get_rollout_engines_and_lock(model_name=model_name)
    ...
    def _trim_train_data(self, data: dict, keep_len: int) -> None:
        old_len = len(data["tokens"])
        if keep_len == old_len:
            return
        if keep_len <= 0:
            raise ValueError(f"Cannot trim train data to non-positive length: {keep_len}")
        for key, value in list(data.items()):
            if isinstance(value, list) and len(value) == old_len:
                data[key] = value[:keep_len]

    def _compute_postprocess_dynamic_global_batch_size(self, data: dict, dp_size: int) -> int:
        num_samples = len(data["tokens"])
        if data.get("dpo_pair_ids"):
            if num_samples % 2 != 0:
                raise ValueError("DPO train data must contain an even number of samples.")
            pair_count = num_samples // 2
            dynamic_pair_count = (pair_count // dp_size) * dp_size
            if dynamic_pair_count == 0:
                raise ValueError(
                    f"Not enough DPO pairs ({pair_count}) for dp_size={dp_size}. "
                    "Need at least one pair per data-parallel rank."
                )
            dynamic_gbs = dynamic_pair_count * 2
            wasted = num_samples - dynamic_gbs
            if dynamic_gbs != self.args.global_batch_size or wasted > 0:
                logger.info(
                    "Dynamic DPO global_batch_size: %s -> %s "
                    "(samples=%s, pairs=%s, dp_size=%s, wasted_samples=%s)",
        ...
        return self._compute_dynamic_global_batch_size(num_samples)

    def _split_pair_aware_partitions(self, data: dict, total_lengths: list[int], dp_size: int):
        pair_ids = data.get("dpo_pair_ids")
        if not pair_ids:
            return None

        if len(pair_ids) != len(total_lengths):
            raise ValueError("dpo_pair_ids length must match tokens length.")
        if len(pair_ids) % 2 != 0:
            raise ValueError("DPO sample count must be even.")

        pair_units: list[list[int]] = []
        pair_lengths: list[int] = []
        i = 0
        while i < len(pair_ids):
            if i + 1 >= len(pair_ids):
                raise ValueError("Found incomplete DPO pair at end of train data.")
            if pair_ids[i] != pair_ids[i + 1]:
                raise ValueError("DPO pair samples must be adjacent before DP splitting.")
            pair_units.append([i, i + 1])
            pair_lengths.append(total_lengths[i] + total_lengths[i + 1])
            i += 2

        if self.args.balance_data:
            pair_partitions = get_seqlen_balanced_partitions(pair_lengths, dp_size, equal_size=True)
        else:
            pair_partitions = [range(i, len(pair_units), dp_size) for i in range(dp_size)]

        partitions = []
        for pair_partition in pair_partitions:
            partition = []
            for pair_idx in pair_partition:
                partition.extend(pair_units[pair_idx])
    ...
    def _split_train_data_by_dp(self, data, dp_size):
        ...
        dynamic_global_batch_size = None
        if self.args.use_dynamic_global_batch_size:
            dynamic_global_batch_size = self._compute_postprocess_dynamic_global_batch_size(data, dp_size)
            self._trim_train_data(data, dynamic_global_batch_size)

        total_lengths = [len(t) for t in data["tokens"]]
        data["total_lengths"] = total_lengths

        pair_aware_partitions = self._split_pair_aware_partitions(data, total_lengths, dp_size)
        if pair_aware_partitions is not None:
            partitions = pair_aware_partitions
