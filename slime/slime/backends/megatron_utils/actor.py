...
from ...utils.ppo_utils import compute_approx_kl
...
_SOLVER_SAMPLE_ROLES = frozenset({"solver", "downstream_solver"})


def _resolve_role_specific_loss_coef(
    args: Namespace,
    metadata: dict | None,
    *,
    extractor_arg_name: str,
    solver_arg_name: str,
    default_arg_name: str,
) -> float:
    role = ""
    if isinstance(metadata, dict):
        role = str(metadata.get("sample_role") or "").strip()

    if role == "extractor":
        extractor_value = getattr(args, extractor_arg_name, None)
        if extractor_value is not None:
            return float(extractor_value)

    if role in _SOLVER_SAMPLE_ROLES:
        solver_value = getattr(args, solver_arg_name, None)
        if solver_value is not None:
            return float(solver_value)

    return float(getattr(args, default_arg_name))


def _attach_train_metadata_loss_coefs(args: Namespace, rollout_data: RolloutBatch) -> None:
    sample_metadata = rollout_data.get("metadata")
    if not sample_metadata:
        return

    rollout_data["sample_roles"] = [
        str(metadata.get("sample_role") or "").strip() if isinstance(metadata, dict) else ""
        for metadata in sample_metadata
    ]

    need_entropy_coefs = any(
        getattr(args, arg_name, None) is not None
        for arg_name in ("solver_entropy_coef", "extractor_entropy_coef")
    )
    need_kl_coefs = any(
        getattr(args, arg_name, None) is not None
        for arg_name in ("solver_kl_loss_coef", "extractor_kl_loss_coef")
    )

    if not need_entropy_coefs and not need_kl_coefs:
        return

    if need_entropy_coefs:
        rollout_data["entropy_coefs"] = [
            _resolve_role_specific_loss_coef(
                args,
                metadata,
                extractor_arg_name="extractor_entropy_coef",
                solver_arg_name="solver_entropy_coef",
                default_arg_name="entropy_coef",
            )
            for metadata in sample_metadata
        ]

    if need_kl_coefs:
        rollout_data["kl_loss_coefs"] = [
            _resolve_role_specific_loss_coef(
                args,
                metadata,
                extractor_arg_name="extractor_kl_loss_coef",
                solver_arg_name="solver_kl_loss_coef",
                default_arg_name="kl_loss_coef",
            )
            for metadata in sample_metadata
        ]
    ...
    def train_actor(self, rollout_id: int, rollout_data: RolloutBatch) -> None:
        ...
        with inverse_timer("train_wait"), timer("train"):
            if self.args.compute_advantages_and_returns:
                ...
                _attach_train_metadata_loss_coefs(self.args, rollout_data)
                rollout_data.pop("metadata", None)
