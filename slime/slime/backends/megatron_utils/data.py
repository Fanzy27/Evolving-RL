...
def get_data_iterator(
    ...
    dpo_pair_ids = rollout_data.get("dpo_pair_ids")
    if dpo_pair_ids:
        if num_local_gbs % 2 != 0:
            raise ValueError("DPO local batch size must be even so pairs are not split across steps.")

        pairs_per_microbatch = int(getattr(args, "alfworld_extractor_dpo_pairs_per_microbatch", 1) or 1)
        pairs_per_microbatch = max(pairs_per_microbatch, 1)
        micro_batch_sample_size = pairs_per_microbatch * 2
        ...
        for i in range(num_steps_per_rollout):
            ...
            for pair_start in range(start, end, 2):
                if pair_start + 1 >= end:
                    raise ValueError("Found incomplete DPO pair inside local rollout step.")
                if dpo_pair_ids[pair_start] != dpo_pair_ids[pair_start + 1]:
                    raise ValueError("DPO pair ids must stay adjacent inside each local rollout step.")

            step_microbatch_count = 0
            for micro_start in range(start, end, micro_batch_sample_size):
                indices = list(range(micro_start, min(micro_start + micro_batch_sample_size, end)))
                if len(indices) % 2 != 0:
                    raise ValueError("DPO microbatch would split a pair.")
                micro_batch_indices.append(indices)
                step_microbatch_count += 1
            num_microbatches.append(step_microbatch_count)
        ...
        if (
            ...
            logger.info(
                "[extractor_dpo/data_iterator] "
                f"num_steps_per_rollout={num_steps_per_rollout} "
                f"global_batch_size={global_batch_size} "
                f"local_batch_size={num_local_gbs} "
                f"global_pairs_per_step={global_batch_size // 2} "
                f"local_pairs_per_step={num_local_gbs // 2} "
                f"pairs_per_microbatch={pairs_per_microbatch} "
