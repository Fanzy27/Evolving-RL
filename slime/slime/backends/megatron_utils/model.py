...
def train_one_step(
    ...
    if mpu.is_pipeline_last_stage(ignore_virtual=True):
        ...
        raw_loss_values = dict(zip(keys, values[1:], strict=False))
        for key, value in zip(keys, values[1:], strict=False):
            if key.endswith("_count"):
                continue
            count_key = f"{key}_count"
            denominator = raw_loss_values.get(count_key, num_samples_or_tokens)
            loss_reduced[key] = (
                value * mpu.get_context_parallel_world_size() / denominator
                if denominator > 0
                else 0.0
            )
