...
def build_per_token_coefficients(
    sample_coefficients: list[float] | None,
    per_sample_tensors: list[torch.Tensor],
    *,
    device: torch.device,
    dtype: torch.dtype,
) -> torch.Tensor | None:
    """Expand per-sample scalar coefficients to local per-token tensors."""

    if sample_coefficients is None:
        return None

    if len(sample_coefficients) != len(per_sample_tensors):
        raise ValueError(
            "Coefficient/sample length mismatch: "
            f"{len(sample_coefficients)} vs {len(per_sample_tensors)}"
        )

    pieces = [
        torch.full((tensor.numel(),), float(coefficient), device=device, dtype=dtype)
        for coefficient, tensor in zip(sample_coefficients, per_sample_tensors, strict=True)
    ]
    if not pieces:
...
_ROLE_METRIC_GROUPS = {
    "extractor": frozenset({"extractor"}),
    "solver": frozenset({"solver", "downstream_solver"}),
}


def _get_role_sample_indices(sample_roles: list[str] | None, accepted_roles: frozenset[str]) -> list[int]:
    if not sample_roles:
        return []
    return [
        idx
        for idx, role in enumerate(sample_roles)
        if str(role or "").strip() in accepted_roles
    ]


def _select_by_indices(values: list[Any] | None, indices: list[int]) -> list[Any] | None:
    if values is None:
        return None
    return [values[idx] for idx in indices]


def _role_metric_count(
    args: Namespace,
    batch: RolloutBatch,
    indices: list[int],
    *,
    ...
    if args.calculate_per_token_loss:
        return sum(
            torch.clamp_min(batch["loss_masks"][idx].sum(), 1)
            for idx in indices
        ).to(device=device, dtype=dtype)
    return torch.tensor(float(len(indices)), device=device, dtype=dtype)


def _sum_role_metric(
    args: Namespace,
    batch: RolloutBatch,
    per_sample_values: list[torch.Tensor],
    indices: list[int],
    *,
    ...
    reducer = get_sum_of_sample_mean(
        [batch["total_lengths"][idx] for idx in indices],
        [batch["response_lengths"][idx] for idx in indices],
        [batch["loss_masks"][idx] for idx in indices],
        args.calculate_per_token_loss,
        args.qkv_format,
        None if max_seq_lens is None else [max_seq_lens[idx] for idx in indices],
    )
    return reducer(torch.cat([per_sample_values[idx] for idx in indices], dim=0))


def _add_role_metric_logs(
    ...
    *,
    ...
    sample_roles = batch.get("sample_roles")
    if not sample_roles:
        return
    if len(sample_roles) != len(batch["response_lengths"]):
        raise ValueError(
            "sample_roles length mismatch: "
            f"{len(sample_roles)} vs {len(batch['response_lengths'])}"
        )

    for role_name, accepted_roles in _ROLE_METRIC_GROUPS.items():
        indices = _get_role_sample_indices(sample_roles, accepted_roles)
        reported_loss[f"{role_name}_{metric_name}"] = _sum_role_metric(
            args,
            batch,
            per_sample_values,
            indices,
        ...
        reported_loss[f"{role_name}_{metric_name}_count"] = _role_metric_count(
            args,
            batch,
            indices,
            device=device,
...
def policy_loss_function(
    ...
    log_probs_per_sample = log_probs
    entropy_per_sample = log_probs_and_entropy["entropy"]
    per_token_entropy_coefs = build_per_token_coefficients(
        batch.get("entropy_coefs"),
        log_probs,
        device=logits.device,
        dtype=logits.dtype,
    )
    per_token_kl_coefs = build_per_token_coefficients(
        batch.get("kl_loss_coefs"),
        log_probs,
        device=logits.device,
        dtype=logits.dtype,
    )
    ...
    # entropy loss
    entropy = torch.cat(entropy_per_sample, dim=0)
    entropy_loss = sum_of_sample_mean(entropy)

    if per_token_entropy_coefs is not None:
        entropy_regularizer = sum_of_sample_mean(entropy * per_token_entropy_coefs)
    else:
        entropy_regularizer = args.entropy_coef * entropy_loss

    loss = pg_loss - entropy_regularizer
    ...
    if args.use_kl_loss:
        ...
        if per_token_kl_coefs is not None:
            kl_regularizer = sum_of_sample_mean(kl * per_token_kl_coefs)
        else:
            kl_regularizer = args.kl_loss_coef * kl_loss

        loss = loss + kl_regularizer
    ...
    _add_role_metric_logs(
        reported_loss,
        args,
        batch,
        metric_name="entropy_loss",
        per_sample_values=entropy_per_sample,
        max_seq_lens=max_seq_lens,
        device=logits.device,
        dtype=logits.dtype,
    )
    ...
    if args.use_kl_loss:
        ...
        kl_per_sample = list(kl.split([sample.numel() for sample in log_probs_per_sample], dim=0))
        _add_role_metric_logs(
            reported_loss,
            args,
            batch,
            metric_name="kl_loss",
            per_sample_values=kl_per_sample,
            max_seq_lens=max_seq_lens,
            device=logits.device,
            dtype=logits.dtype,
        )
